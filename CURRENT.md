...
- **Recorded milestone:** 001 — Foundation. For owner direction, status or
  authorization questions, read the backlog's [standing rules](docs/backlog.md#standing-rules)
  and [current governing owner decisions](docs/backlog.md#current-governing-owner-decisions),
  then the exact relevant cycle/source. Do not load the full history by default.
- **Research:** [workspace and protocol quick reference](research/README.md#protocol-quick-reference).
  The portfolio architecture is Adopted; the protocol remains Draft. Check
  each target's own status and the protocol version used. The workspace defines
  the historical/current boundary; older records are not current-system findings
  merely because they are linked here.
- **Implementation/publication:** `AI-Rights` controls substantive meaning;
